...
// const { sendPasswordResetEmail } = require("../services/emailService"); // 👈 Commented out if service doesn't exist

const router = express.Router();

// ============================================================================
// ENHANCED USER ROUTES WITH MISSING GET FUNCTIONALITY
// ============================================================================
...
router.post("/signup", async (req, res) => {
  try {
    ...
    // ✅ IMPROVED: Better field consistency
    const userData = {
      _id: user._id,
      fullName: user.fullName, // ✅ Consistent with model
...
// ✅ ENHANCED FORGOT PASSWORD - MOVED TO TOP
router.post("/forgot-password", async (req, res) => {
  try {
    ...
    res.status(200).json({
      success: true,
      message: "If an account with that email exists, we've sent a password reset link.",
      ...(process.env.NODE_ENV === "development" && { resetToken }), // return for testing only
    });
  } catch (error) {
...
// ✅ ENHANCED RESET PASSWORD - MOVED TO TOP
...
// ✅ ADDED: GET ALL USERS (Admin functionality)
...
// ✅ ADDED: GET USER PROFILE - MOVED BEFORE /:userid
  ...
  try {
    ...
    res.status(200).json({
      ...
      data: {
        ...
        profileCompleteness: 85 // Can be calculated based on filled fields
      }
    });
...
// ✅ ADDED: UPDATE USER PROFILE - MOVED BEFORE /:userid
...
// ✅ MOVED TO END: GET USER BY ID - MOST GENERIC ROUTE LAST
router.get("/:userid", async (req, res) => {
  try {
    ...
    if (includeStats === 'true') {
      try {
        // Get user's order statistics (if Order model exists)
          ...
          const orderStats = await Order.aggregate([
            { $match: { userId: new mongoose.Types.ObjectId(userid) } },
            {
              $group: {
                _id: null,
                totalOrders: { $sum: 1 },
                totalSpent: { $sum: '$totalAmount' }, // ✅ FIXED: Use totalAmount
                avgOrderValue: { $avg: '$totalAmount' }, // ✅ FIXED: Use totalAmount
